...
| `dbus` | no | `zbus`, `serde_json` | `usbeehive::dbus` module + `usbeehived` daemon publishing `org.usbeehive.Devices3` — implies `watch` |
...
| `src/dbus.rs` | `org.usbeehive.Devices3` interface + `DeviceEntry` / `PdoEntry` / `DiagnosticEntry` wire types |
