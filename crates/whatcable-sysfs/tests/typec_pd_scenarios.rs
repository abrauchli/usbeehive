...
use whatcable_core::diagnostic::Bottleneck;
use whatcable_core::pd::{CableCurrent, CableSpeed};
...
fn typec_charging_with_pps_charger_and_5a_active_cable() {
    ...
    let pps = pd
        .source_capabilities
        .iter()
        .find(|p| matches!(p.r#type, whatcable_core::power::PdoType::Pps))
    ...
    let fixed: Vec<_> = pd
        .source_capabilities
        .iter()
        .filter(|p| matches!(p.r#type, whatcable_core::power::PdoType::FixedSupply))
        .collect();
