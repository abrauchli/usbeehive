...
use whatcable_core::pd::{cable_current_label, cable_speed_label};
use whatcable_core::summary::{Category, DeviceSummary};
use whatcable_core::usbclass;
...
mod tests {
    use super::*;
    use whatcable_core::summary::Status;
    ...
    fn typec_summary_serializes() {
        let port = whatcable_core::typec::TypeCPort {
            port_number: 0,
    ...
    fn text_output_renders_charger_profiles() {
        use whatcable_core::power::{PdoType, PowerDataObject, PowerDeliveryPort};
        use whatcable_core::typec::{TypeCPartner, TypeCPort};
        let port = TypeCPort {
            port_number: 0,
